...
<?php
    ...
    {
        header("Location: ".$hlpr->core_link('not-found')."");
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") 
    {
        ...
        ) 
        {
            ...
            else
            {
                ...
                $registration_status = $user->create_admin();
                
                if($registration_status == 1)
                {
                    header("Location: ".$hlpr->core_link("login?email=".$user->email)."");
                    exit;
                }
            }
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <form class="auth-form" method="post" action=" <?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                    <h5>Registration Admin Form</h5>
                    <hr/>
                    <?php 
                    echo $user->_admin_exists();
                    if(!$user->_admin_exists()) 
                    {
                    ?> 
                        <p class="text-muted" style="padding:0; margin:0;"><small>Note! First Admin user will responsible for all site settings including adding new admin users.</small></p>
                        <p class="text-muted" style="padding:0; margin:0;"><small>After admin user created this page will not be available.</small></p> 
                        <p class="text-muted" style="padding-top:0; margin-top:0;"><small>All other admin users should be added from <span class="badge badge-dark"> profile/site-settings</span> page.</small></p> 
                    <?php 
                    }
                    ?>
